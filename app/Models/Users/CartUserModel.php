...
class CartUserModel
{
    ...
    public function addCartModel()
    {
        ...
        $sql = "select cart_detail.*, products.name, products.image_main, products.price, products.price_sale from cart_detail join
        products on cart_detail.product_id = products.id where cart_detail.cart_id = :cart_id";
        $stmt = $this->db->pdo->prepare($sql);
        $stmt->bindParam(':cart_id', $cartId);
        $stmt->execute();
    ...
    public function updateCartModel()
    {
        ...
        $action = $_POST['action']; // increase(tang), decrease(giam), deleted(xoa)
